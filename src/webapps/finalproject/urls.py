...
urlpatterns = [
    ...
    url(r'^photo/(?P<id>\d+)$', finalproject.views.get_photo, name='photo'),
    ...
    url(r'^user/password/reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        password_reset_confirm, {'template_name': 'password_reset_confirm.html',
                                 'post_reset_redirect': '/user/password/done/'}, name='password_reset_email'),
    ...
    url(r'^api_resume', finalproject.views.api_resume, name='api_resume'),
