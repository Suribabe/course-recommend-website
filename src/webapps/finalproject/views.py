...
def get_profile(request):  # This function triggers when user click on the profile link
    ...
    try:
        ...
        if user_profile.location:
            context['location'] = eval(user_profile.location)['name']
    except:
        context['social'] = ''
...
def home(request):
    ...
    # context['courses'] = recommend_by_similarity(request)
...
def home_linkedin(request):
    ...
    try:
        profile = User_profile(user=request.user, location=user_profile['location'],
                               industry=user_profile['industry'], picture_url=user_profile['pictureUrl'])
        profile.save()
    finally:
        context['courses'] = recommend_by_history(user)
        return render(request, "main.html", context)
...
def recommend_by_history(user):
    ...
    if search_term1 is not None:
        courses1 = Course.search_course_by_name(search_term1)[:4]
    ...
    if search_term2 is not None:
        courses2 = Course.search_course_by_name(search_term2)[:2]
...
def get_random_courses():
    ...
    for i in range(1, 7):
        next_index = random.randint(1, 2052)
        while next_index in courseDict:
            next_index = random.randint(1, 2052)
        next_course = Course.objects.get(id=next_index)  # All courses are download to our database
...
def get_photo(request, id):
    profile = get_object_or_404(User_profile, user_id=id)
...
def sort_by_rating(request):
    context = {}
    course_list = Course.objects.all().order_by('-ratings__average', 'course_name')
    course = []
    for i in range(6):
